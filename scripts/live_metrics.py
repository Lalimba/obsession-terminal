...
def coinglass_get(path, params=None):
    ...
    print("COINGLASS URL:", url)
    print("COINGLASS HEADER NAMES:", list(headers.keys()))
    print("COINGLASS PARAMS:", params or {})

    r = requests.get(url, headers=headers, params=params or {}, timeout=TIMEOUT)

    print("COINGLASS STATUS:", r.status_code)
    print("COINGLASS RESPONSE:", r.text[:1000])
...
def payload_last_value(payload):
    ...
    for row in rows:
        ...
        if ts_num is not None:
            dt = pd.to_datetime(
                int(ts_num),
                unit="ms" if ts_num > 10_000_000_000 else "s",
                utc=True,
            )
        else:
...
def latest_funding():
    try:
        ...
        print("Funding fetch error:", str(e))
        return None
...
def latest_etf_flow():
    try:
        ...
        print("ETF flow fetch error:", str(e))
        return None
...
def upsert_current_metrics(row):
    ...
    print("SUPABASE RESPONSE:", r.text[:1000])
    r.raise_for_status()
...
def main():
    ...
    print("ROW TO UPSERT:", row)
    upsert_current_metrics(row)
    print("current_metrics updated successfully")
